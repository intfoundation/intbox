...
const getMask = (tag: string,t: ContextApi['t'], contract: string, tokenid: number)=>{
  ...
  if(tag === 'hot'){
    return (
      <StyledCardImgMask>
        <StyledLink to={`/nft/onsale/${contract}/${tokenid}`}>
          <StyledCardImgBtn>
            <div>{t('Remove')}</div>
            <img src={toRight} alt=''/>
          </StyledCardImgBtn>
...
const NftCardOnSale:React.FunctionComponent<props>
  = ({
     showTag=true,
     tag='hot',
      curNft
  ...
  return (
    <UiItemBox>
      <UiItemBoxContainer>
        {showTag && tagView}
        <CardImg curNft={curNft} tag={tag} type="onsale"/>
        <CardInfoRow curNft={curNft} tag={tag} type="onsale"/>
      </UiItemBoxContainer>
    </UiItemBox>
  )
}
